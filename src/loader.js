...
import vueLoader from 'jud-vue-loader'
...
function loader (source) {
  ...
  const options = this.options.jud || {}
  ...
  if (frag.deps.length) {
    for (const dep of frag.deps) {
      const filepath = path.resolve(path.dirname(resourcePath), `${dep}.ju`)
...
`
  if (isEntry) {
    output += `
__jud_bootstrap__('@jud-component/${name}'`
  + (frag.config.length > 0 ? `,__jud_config__` : ',undefined')
  + (frag.data.length > 0 ? `,__jud_data__` : ',undefined')
+ `)`
  }
